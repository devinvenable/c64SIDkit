...
VOICE_MAP = {1: Voice.ONE, 2: Voice.TWO, 3: Voice.THREE}
...
def render_patch_resid(
    ...
    has_filter = hasattr(patch, 'filter_mode') and patch.filter_mode != "off"
    if has_filter:
        # Filter cutoff: 11-bit value. Map our 8-bit cutoff to 11-bit.
        cutoff_11 = patch.filter_cutoff << 3
        ...
        res_filt = (patch.filter_resonance << 4) | voice_filter_bit
        ...
        mode_vol = mode_bits.get(patch.filter_mode, 0x10) | 0x0F
    ...
    for frame in range(n_frames):
        ...
        # Handle pitch sweep
        if patch.has_sweep and frame < patch.duration_frames:
            frac = frame / max(1, patch.duration_frames - 1)
            start_freq = patch.frequency
            end_freq = patch.sweep_target
        ...
        if has_filter and patch.filter_cutoff_sweep > 0 and frame < patch.duration_frames:
            frac = frame / max(1, patch.duration_frames - 1)
            start_cut = patch.filter_cutoff
            end_cut = patch.filter_cutoff_sweep
